...
async function mangatoons(query) {
    try {
        const { data } = await axios.get(`https://mangatoon.mobi/en/search?word=${query}`);
        const $ = cheerio.load(data);
        const hasil = [];

        $('.recommend-item').each((index, element) => {
            const result = {
                judul: $(element).find('.recommend-comics-title > span').text() || 'Tidak ada',
                genre: $(element).find('.comics-type > span').text().trim(),
                link: 'https://mangatoon.mobi' + $(element).find('> a').attr('href'),
                thumbnail: $(element).find('.comics-image > img').data('src')
            };
            hasil.push(result);
        });
...
async function mangatoonpopuler() {
    try {
        const { data } = await axios.get('https://mangatoon.mobi/en/genre/comic');
        const $ = cheerio.load(data);
        const hasil = [];

        $('.genre-content .items .item').each((index, element) => {
            const result = {
                judul: $(element).find('.content-title > span').text() || 'Tidak ada',
                genre: $(element).find('.content-tags > span').text().trim().replace(/\s+/g, ' '),
                link: 'https://mangatoon.mobi' + $(element).closest('a').attr('href'),
                thumbnail: $(element).find('.content-image img').data('src')
            };
            hasil.push(result);
        });
